  ...
  <div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
            ...
            <form class="form-horizontal">
				...
				<input id="idorder" name="id_order" type="hidden">
                    ...
                    <input type="text" class="form-control" id="biaya" name="biaya" data-inputmask-inputformat="999.999.999.999" data-mask readonly>
                  </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-12">
						<div class="input-group">
							<select name="is_lunas" class="form-control" id="is_lunas">
...
<script>
	...
	$(document).ready(function () {
		...
		function tampil_data() {
			...
			$.ajax({
				type: "GET",
				url: "<?php echo base_url()?>internal/getPengiriman",
				async: true,
				dataType: "json",
				success: function (data) {
					...
					for (i = 0; i < data.length; i++) {
						...
						var stat='';
						var lunas='';
						if(data[i].status=='pickup'){
							stat='Penjemputan Barang';
						...
						html +=
							...
							"<button class='btn btn-warning btn-sm' data-idorder='"+data[i]['id_order']+"' data-id_cl='"+data[i]['id_cl']+"' data-keterangan='"+data[i]['keterangan_pembayaran']+"' data-jns_pembayaran='"+data[i]['jns_pembayaran']+"' data-pengirim='"+data[i]['pengirim']+"' data-biaya='"+data[i]['biaya']+"' data-status='"+data[i]['status']+"' data-lunas='"+data[i]['is_lunas']+"' onclick='$(\"#ModalEdit\").modal(\"show\"); $(\"#biaya\").val($(this).data(\"biaya\")); $(\"#id_cl\").val($(this).data(\"id_cl\")); $(\"#status\").val($(this).data(\"status\")); $(\"#jns_pembayaran\").val($(this).data(\"jns_pembayaran\"));  $(\"#is_lunas\").val($(this).data(\"lunas\")); $(\"#namaedit\").text(\"Memproses \"+$(this).data(\"pengirim\")+\" - \"+$(this).data(\"keterangan\")); $(\"#idorder\").val($(this).data(\"idorder\")); proseskiriman();'><i class='fas fa-edit'>Proses</i></button>" +
							" " +
							"</td>" +
							"</tr>";
		...
		$('#btn_update').on('click',function(){
            ...
            $.ajax({
                type : "POST",
                url  : "<?php echo base_url('internal/updatepengiriman')?>",
                data : {id_order:id, biaya:biaya,status:status,is_lunas:is_lunas,id_cl:id_cl},
                success: function(data){
                    $('[name="status"]').val("");
                    $('[name="biaya"]').val("");
                    $('[name="id_order"]').val("");
	...
	function proseskiriman(){
			var status=$('#status').val();
			var jns=$('#jns_pembayaran').val();
			var lunas=$('#is_lunas').val();

			$('#is_lunas option[value="checking"]').hide();

			
			if(lunas=='yes'){
				$('#is_lunas option[value="no"]').hide();
				$('#status option[value="cancel"]').hide();
			}else{
